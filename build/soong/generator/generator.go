...
func init() {
	android.RegisterModuleType("mdroid_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = mdroidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = mdroidExpandVariables(ctx, dep_file)
	...
	cmd := mdroidExpandVariables(ctx, String(g.properties.Cmd))
